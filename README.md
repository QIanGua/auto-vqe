...
- **`core/controller.py` (The Overseer)**: Manages experimental budgets (max runs, wall-clock time) and monitors search progress via stopping rules (no-improvement, failure limits).
...
- **`core/controller.py`（实验监考官）**：管理实验预算（最大运行次数、耗时上限），并通过停止规则（长时间无改进、持续失败）监控搜索进度。
